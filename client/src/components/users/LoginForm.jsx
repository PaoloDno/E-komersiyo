...
const LoginForm = () => {
  ...
  const { isLoading, error, isAuthenticated, user, token } = useSelector((state) => state.auth);
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-gray-800 p-8 rounded-lg shadow-md w-fit">
      ...
      <form onSubmit={handleLogin} className="w-fit max-w-md border border-cyan-200 p-5 px-10 rounded-lg">
        ...
        <button
          type="submit"
          className={`w-full p-3 text-white rounded transition-colors ${ isLoading || !username || !password
